    ...
    <div class="wrapper mt-3">
            <div class="header">
                    <div class="float-left text-center" style="max-width:500px;margin-left:50px;padding-top:40px">
                        ...
                        <p>Laporan Kegiatan Per tanggal <b>{{ tanggal_indonesia($tgl_dari, false) }}</b> sampai <b>{{ tanggal_indonesia($tgl_sampai, false) }}</b></p>
                    </div>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($kegiatan as $no => $k)
                                    <tr>
                                        ...
                                        @if ( $status == 'semua' )
                                            <td>{{ $k->status }}</td>
                                        @endif
                                        <td>{{ tanggal_indonesia($k->deadline) }}</td>
                                        <td>{{ tanggal_indonesia($k->tanggal_dilaksanakan) }}</td>
                                        <td>{{ format_uang($k->estimasi_biaya) }}</td>
                                ...
                                <tr>
                                    ...
                                    @endif
                                    <td>Rp. <b>{{ format_uang($kegiatan->sum('estimasi_biaya'))}}</b></td>
                                </tr>
                            </tbody>
                        </table>
